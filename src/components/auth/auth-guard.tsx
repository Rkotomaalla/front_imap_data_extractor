...
export const AuthGuard = ({
	children,
	fallback = null,
	check, // juste le rôle à vérifier : "admin" ou "user"
}: AuthGuardProps) => {
	const checkFn = useAuthCheck();

	// Vérification simple : si "check" n'est pas fourni, on autorise par défaut
	const hasAccess = check ? checkFn.check(check as "admin" | "user") : true;
